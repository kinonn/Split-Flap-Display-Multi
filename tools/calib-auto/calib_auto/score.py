"""Scoring for the benchmark and calibration checks (pure functions).

The score of one row is the number of character positions where the read
differs from the golden content — lower is better. The recorded prior
read (``saw``, produced by a previous reader) is scored exactly the
same way, and the run report compares the two.

Normalization is applied to the read, ``saw`` and truth alike:
...
- pad short / truncate long values to the row width with spaces, flagging
  the row ``adjusted`` — adjusted rows are still counted, never silently
  dropped.
"""
...
def score_reading(read: str | None, want: str, saw: str, width: int) -> dict:
    """Normalize + compare one row's read and baseline against the truth.

    ``read`` is the model answer (None when the read failed). Returns
    the comparable, JSON-serializable pieces of a row.
    """
...
def summarize(rows: list[dict], width: int) -> dict:
    ...
    Rows with ``error`` set (invalid line, photo unreadable, model
    failure) are excluded from every accuracy average and counted
    separately.
    """
    ...
    for r in ok:
        ...
        if a < b:
            ...
            h2h["equal"] += 1
        deltas.append(b - a)  # positive = model better than baseline
    if deltas:
