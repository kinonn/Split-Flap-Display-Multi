"""VLM read path: one display photo -> width-reconciled per-module reading.

The reading contract shared by both recognizers of this tool (the VLM
reader here and the CNN reader in ``cnn_reader.py``): ``read(...) ->
Reading``, where ``Reading.modules`` carries one ``ModuleReading`` per
module with char, condition, confidence and source. The calibration
loop is written against exactly this interface, so the approach (CNN or
VLM) is a configuration choice, not a code path.
...
character it sees and the flap condition; this module reconciles the
answer against the known module count (blanks included) and flags
anything it had to infer.
"""
...
class ModuleReading:
    ...
    condition: str
    confidence: float
    source: str = "vlm"  # "vlm" | "cv" | "classifier" | "inferred" | "error"
    expected: str = ""
...
class Reading:
    ...
    # A reader that knows its own geometry failed (display not located, or
    # the module grid does not fit the display) says so here instead of
    # letting plausible-looking characters drive a calibration: the loop
    # treats an unreliable reading as a reader failure, so a systematically
    # bad framing aborts early with the note instead of grinding through a
    # sweep on garbage. Readers that cannot judge themselves (the VLM) keep
    # the default.
