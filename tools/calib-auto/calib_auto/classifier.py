...
The dependency-free backend of the CNN approach. The rules of this
module:

- **Verified truth only.** Training data comes from glyph caches built
  by ``glyphs.py``, which only ever reads ``verified`` golden entries.
  Pending pre-fills never train or score anything.
- **Photo-disjoint evaluation.** Accuracy is never reported on photos
  the bank trained on: ``split="photo"`` holds out a hash-selected
  fifth of the photos, ``split="set"`` holds out a whole golden set
...
- **Two numbers, both true.** ``bank_acc`` runs the classifier on every
  cell (blanks included as their own class); ``sim_acc`` simulates the
  reader — with ``--blank-gate`` OpenCV decides blanks and the bank
  classifies the rest, otherwise every cell goes to the bank — and
  reports whole-row mismatches, the unit the calibration loop scores in.

The bank itself is deliberately dependency-free: per class the mean of
unit-normalized grayscale rasters, prediction by cosine similarity,
``(char, confidence, margin)`` out. It is fast, tiny and on clean
curated crops already strong; the CNN backend (``train_cnn.py``)
exists for glare/pose robustness and is a drop-in replacement behind
the same interface.

CLI::

    python -m calib_auto.classifier build [--size 48] [--sets ...]
    python -m calib_auto.classifier eval  [--split all|photo|set] [--holdout NAME]
    python -m calib_auto.classifier read  PHOTO [--modules 12]
"""
...
from . import glyphs, paths, segment

BANK_FILE = "bank.npz"
DEFAULT_SIZE = 48
BLANK = " "
# The display's drum character set (must match config.DEFAULT_CHARSET).
# Curated labels outside it are typos/transients (one lowercase 'v' was
# found in an early set); they are excluded from training and counted in
# the bank meta so the data problem stays visible instead of becoming a
# phantom class.
...
def bank_path() -> str:
    return os.path.join(paths.training_root(), BANK_FILE)
...
class GlyphBank:
    """Per-class mean templates on unit-normalized grayscale rasters."""
    ...
    def save(self, path: str, meta: dict | None = None) -> str:
        ...
        payload = {
            ...
            "charset": CHARSET,
            "excluded_labels": dict(self.skipped_labels),
        }
...
def load_caches(sets: list[str] | None = None) -> list[dict]:
    """Load every available glyph cache (optionally a subset)."""
    names = glyphs.available_sets()
    if sets:
        wanted = set(sets)
        ...
        if missing:
            raise BankError(
                ...
                + " — run: python -m calib_auto.glyphs")
    return [glyphs.load_cache(name) for name in names]


def train_bank(*, sets: list[str] | None = None,
               size: int = DEFAULT_SIZE,
               out: str | None = None) -> GlyphBank:
    caches = load_caches(sets)
    if not caches:
        raise BankError("no glyph caches found — run: python -m calib_auto.glyphs")
    bank = GlyphBank(size).fit(caches)
    styles = {str(c.get("style", "none")) for c in caches}
    bank.save(out or bank_path(), meta={
...
# -- blank/glyph gating policy --------------------------------------------------

def classify_raster(bank, raster, *, blank: bool,
                    blank_gate: bool = False,
                    floor: float = GLYPH_BRIGHT_FLOOR
                    ) -> tuple[str, float, float, str]:
    """Canonical raster + blank flag -> (char, conf, margin, source).

    With ``blank_gate`` off (the default) every cell goes to the model:
    the ``blank`` ink-test flag is ignored and a missing/dim raster is a
    classifier blank (``source="classifier"`), never an OpenCV short
    circuit. With ``blank_gate`` on, no raster or a dim raster *that the
    ink test also called blank* is a blank cell (no classifier call);
    everything bright is classified, including cells the ink test called
    blank (the tiny '.'/'\\'' glyphs).
    """
    if raster is None or not raster.any():
        source = "cv" if blank_gate else "classifier"
        return BLANK, 1.0, 1.0, source
    if blank_gate and blank and float(np.percentile(raster, 99)) < floor:
...
def classify_cell(bank, crop, *, blank: bool,
                   blank_gate: bool = False,
                   floor: float = GLYPH_BRIGHT_FLOOR
                   ) -> tuple[str, float, float, str]:
    """Live module crop -> gating + classification (read path)."""
    raster = segment.canonical_glyph(crop, glyphs.GLYPH_SIZE)
    return classify_raster(bank, raster, blank=blank,
                           blank_gate=blank_gate, floor=floor)
...
def evaluate(*, sets: list[str] | None = None,
             size: int = DEFAULT_SIZE, split: str = "all",
             holdout: str | None = None, photo_frac: float = 0.2,
...
def read_image(bank: GlyphBank, path: str, total: int = 12,
               blank_gate: bool = False) -> dict:
    ...
    for i, crop in enumerate(segment.crop_modules(image, boxes)):
        blank = segment.is_blank(crop) if blank_gate else False
        char, conf, margin, source = classify_cell(
            bank, crop, blank=blank, blank_gate=blank_gate)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--holdout", default=None,
                        help="golden set to hold out for --split set")
    ...
    parser.add_argument("--blank-gate", action="store_true",
                        help="enable the OpenCV blank gate (default: off, "
                             "every cell goes to the model)")
    args = parser.parse_args(argv)

    try:
        if args.command == "build":
            bank = train_bank(sets=args.sets, size=args.size)
            path = bank_path()
        ...
        if args.command == "eval":
            metrics = evaluate(sets=args.sets, size=args.size,
