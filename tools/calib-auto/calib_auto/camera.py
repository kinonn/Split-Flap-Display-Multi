...
class Camera:
    ...
    # (or this expires). A fixed 0.5 s often ends mid-hunt on backends
    # that ignore the exposure lock, so the check would sample the
    ...
    def _lock_exposure(self) -> None:
        ...
        Flipping to manual and freezing the current value made the live
        view darker than doing nothing at all on drivers whose manual
        range is darker than their AE (AE adds gain).
        """
        cap = self.cap
    ...
    def _auto_exposure(self) -> None:
        ...
        Forcing manual unconditionally made dark scenes DARKER on
        drivers whose manual range caps above their AE (AE adds gain):
        on the affected camera the mean dropped from 32 (auto) to 8
        (manual max). Full-open only; the live view's quick open just
        ensures AE via _lock_exposure.
        """
        cap = self.cap
        assert cap is not None
        diag: dict = {"steps": 0, "mean_before": None, "mean_after": None,
