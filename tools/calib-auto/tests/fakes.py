"""Fakes for calib-auto tests: no camera, display or network needed."""
...
class SimReader:
    ...
    ``source`` defaults to "vlm"; pass "classifier" to model the CNN
    reader (both are trusted by the calibrator).
    """

    def __init__(self, display: FakeDisplay, annotate: bool = False,
                 confidence: float = 0.99, source: str = "vlm",
    ...
    def read(self, jpeg: bytes, total: int, expected: str = "",
        ...
        for i in range(total):
            ...
            modules.append(ModuleReading(i, char, cond, self.confidence,
                                         self.source, cmd))
        return Reading(modules, raw_count=total, unreliable=self.unreliable,
                       unreliable_note=("module grid does not fit"
                                        if self.unreliable else ""))
