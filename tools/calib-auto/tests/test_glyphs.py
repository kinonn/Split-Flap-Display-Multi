"""Glyph cache tests: extraction from golden sets, skips, roundtrip."""
...
def test_extract_set_skips_with_reasons(env):
    make_baseline_set(env, "s3", {"ok.png": CHUNKY, "gone.png": CHUNKY,
                                  "bad.png": CHUNKY})
    images = env / "golden" / "s3" / "images"
    os.remove(images / "gone.png")
    (images / "bad.png").write_bytes(b"definitely not a png")
    golden.update_entry("s3", "ok.png", content=CHUNKY[:5])  # width mismatch
...
def test_cache_save_load_roundtrip(env):
    ...
    npz_path, json_path = glyphs.save_cache(payload)
    assert os.path.isfile(npz_path) and os.path.isfile(json_path)
    loaded = glyphs.load_cache("s4")
...
def test_available_sets_excludes_bank_artifact(env):
    ...
    glyphs.save_cache(glyphs.extract_set("s5"))
    assert glyphs.available_sets() == ["s5"]
    ...
    np.savez(os.path.join(glyphs.cache_dir(), "bank.npz"),
...
def test_load_cache_missing_raises(env):
    with pytest.raises(FileNotFoundError):
        glyphs.load_cache("nope")


def test_build_with_subset_builds_only_those(env):
    make_baseline_set(env, "s1", {"p1.png": CHUNKY})
    make_baseline_set(env, "s2", {"p1.png": CHUNKY})
    result = glyphs.build(sets=["s1"])
    assert list(result["sets"]) == ["s1"]
    assert glyphs.available_sets() == ["s1"]
...
def test_build_reports_no_train_sets(env):
    ...
    golden.create_set(str(src), name="s7", image_format="keep")  # pending

    result = glyphs.build()
    assert result["cells"] == 12
    assert "s6" in result["sets"]
    assert result["no_train_set"] == ["s7"]
    assert glyphs.available_sets() == ["s6"]
