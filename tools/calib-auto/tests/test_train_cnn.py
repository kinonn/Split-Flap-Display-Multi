...
def test_save_load_model_roundtrip(env, tmp_path):
    make_baseline_set(env, "s1", {"p1.png": "AMAMAMAMAMAM"})
    glyphs.build()
    result = train_cnn.train(epochs=4, size=32, batch=16,
                             all_data=True, verbose=False)
    ...
    pred = loaded.predict_raster(glyphs.load_cache("s1")["crops"][0])
    assert pred is not None and pred[0] in result["classes"]


def test_train_reports_class_counts(env, tmp_path):
    # per-class cells the model trained on — what the /train distribution
    # table shows as the "model" column (persisted into cnn.json)
    make_baseline_set(env, "s1", {"p1.png": "AMAMAMAMAMAM",
                                  "p2.png": "AMAMAMAMAMAM"})
    glyphs.build()
    result = train_cnn.train(epochs=2, size=32, batch=16,
                             all_data=True, verbose=False)
    assert result["class_counts"] == {"A": 12, "M": 12}
    assert sum(result["class_counts"].values()) == result["train_cells"]
    assert result["train_cells"] == result["cells"] == 24
    path = str(tmp_path / "cc.pt")
    train_cnn.save_model(result, path, meta={"trained": "test"})
    with open(os.path.splitext(path)[0] + ".json",
              encoding="utf-8") as fh:
        assert json.load(fh)["class_counts"] == {"A": 12, "M": 12}
