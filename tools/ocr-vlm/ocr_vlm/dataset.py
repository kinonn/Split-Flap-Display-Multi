...
A *baseline set* (see ``ocr_vlm/curate.py``) is the corrected variant:
``baseline.jsonl`` lines carry the human-verified ``content`` as truth
and the previous reader's ``prior_read`` as the baseline, photos live
in an ``images/`` subfolder, and every entry has a ``status``. Pending
...
# A curated baseline set's manifest (see ocr_vlm/curate.py).
...
_DISCOVER_ROOTS = (
    ...
    # Baseline sets live inside this project; a default-name discover
    # simply skips them (no reads.jsonl inside a set).
    ("ocr-vlm", "baselines"),
)
...
class Dataset:
    ...
    kind: str = "reads"                # "reads" | "baseline"
    verified: int = 0                  # baseline sets: entry status counts
    pending: int = 0
...
def photo_path(directory: str, name: str) -> str | None:
    ...
    Baseline sets keep their copies in an ``images/`` subfolder, so the
...
def resolve_dataset_file(directory: str, filename: str = "reads.jsonl") -> str:
    """The manifest that exists: ``filename``, else ``baseline.jsonl``.

    The fallback applies to the *default* name only: pointing the UI at
    a baseline-set folder with ``reads.jsonl`` still selected should
    just work, while a deliberately typed different name is honoured
    (a typo then surfaces in Validate instead of silently loading
    something else).
    """
    ...
    if os.path.isfile(os.path.join(directory, filename)):
        return filename
    if filename == "reads.jsonl" and os.path.isfile(
...
def load_dataset(directory: str, filename: str = "reads.jsonl",
    ...
    Handles both formats: ``reads.jsonl`` (``want`` + ``saw``) and a
    baseline set's ``baseline.jsonl`` (``content`` = human truth,
    ``prior_read`` = baseline). For baseline sets ``verified_only``
    keeps pending entries out of ``records`` (counted in
    ``skipped_pending``); with it off they load but are flagged.
    ...
    ds.kind = "baseline" if filename == BASELINE_FILE else "reads"
    baseline = ds.kind == "baseline"
    path = ds.path
    try:
    ...
    for lineno, line in enumerate(lines, 1):
        ...
        if not isinstance(item, dict):
            ...
            continue
        status = str(item.get("status") or "pending") if baseline else ""
        if baseline:
            if status == "verified":
                ds.verified += 1
            else:
        ...
        rec = _record_from_item(len(ds.records), lineno, item, module_count,
                                baseline=baseline)
        if baseline and status != "verified":
...
def _record_from_item(index: int, lineno: int, item: dict,
                      module_count: int, baseline: bool = False) -> Record:
    photo = item.get("photo")
    # Baseline entries keep the human truth in ``content`` and the
    # previous reader's answer in ``prior_read``; reads.jsonl uses the
    # ``want``/``saw`` pair. Normalize both into the Record fields.
    want = item.get("content") if baseline else item.get("want")
    saw = item.get("prior_read", "") if baseline else item.get("saw", "")
    kind_word = "content" if baseline else "want"
    fatal: list[str] = []
    issues: list[str] = []
