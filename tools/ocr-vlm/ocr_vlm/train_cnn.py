"""Local glyph classifier: CNN backend (Stage C).

The template bank proved the approach — geometry-normalized glyph
rasters classify at ~95% reader-simulated accuracy with leave-one-shoot-out
...
crops can learn those invariances, plus the ``space`` class itself
...
- **Photo-disjoint validation.** Photos are split by a deterministic
  ...
  trained on. ``--all`` trains the deployment artifact on everything
  after the split run reports honest numbers.
...
CLI::
    ...
    python -m ocr_vlm.train_cnn [--data DIR] [--epochs 30] [--size 64]
                                [--batch 128] [--lr 1e-3]
...
from . import classifier, glyphs, segment
...
def _to_tensor(batch: np.ndarray):
    import torch

    return torch.from_numpy(batch.astype(np.float32)).unsqueeze(1)
...
def train(data: str | None = None, *, sets: list[str] | None = None,
    ...
    caches = classifier.load_caches(data, sets)
    if not caches:
        raise classifier.BankError(
            "no glyph caches found — run: python -m ocr_vlm.glyphs")
    gd = GlyphData(caches)
    if len(gd.classes) < 2:
    ...
    model = build_model(len(gd.classes), input_size=size)
    ...
    crit = nn.CrossEntropyLoss(
        weight=torch.from_numpy(weights), label_smoothing=0.05)
    ...
    for epoch in range(max(1, epochs)):
        ...
        for start in range(0, len(order), batch):
            ...
            x = _prep_batch(np.stack([augment(gd.rasters[i], rng)
                                      for i in idx]), size)
            y = torch.from_numpy(gd.targets[idx])
            optimizer.zero_grad()
            loss = crit(model(_to_tensor(x)), y)
            loss.backward()
            optimizer.step()
            scheduler.step()
    ...
    if not all_data and len(test_idx):
        ...
        metrics["seed"] = seed
    result = {"model": model, "classes": gd.classes, "metrics": metrics,
...
def _predict_idx(model, rasters: np.ndarray, size: int, batch: int):
    ...
    with torch.no_grad():
        for start in range(0, len(rasters), batch):
            ...
            out[start:start + len(chunk)] = (model(_to_tensor(x))
                                             .argmax(dim=1).numpy())
    return out
...
def evaluate_model(model, gd: GlyphData, test_idx: np.ndarray, size: int,
    ...
    with torch.no_grad():
        for start in range(0, len(test_idx), batch):
            ...
            probs = probs_fn(model(_to_tensor(x))).numpy()
...
class CnnClassifier:
    ...
    @classmethod
    def load(cls, path: str) -> CnnClassifier:
        import torch
    ...
    def predict_raster(self, raster) -> tuple[str, float, float] | None:
        ...
        with torch.no_grad():
            probs = torch.nn.functional.softmax(
                self.model(_to_tensor(x)), dim=1)[0].numpy()
...
def model_path(data: str | None = None) -> str:
    return os.path.join(glyphs.cache_dir(data), MODEL_FILE)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ocr_vlm.train_cnn",
        description="Train the CNN glyph backend on curated crops")
    ...
    if not args.train_all_only:
        ...
        result = train(args.data, sets=args.sets, epochs=args.epochs,
        ...
        if args.no_final:
            path = save_model(result, model_path(args.data),
                              meta={"metrics": metrics,
                                    "trained": "photo-split"})
            print(f"saved (split model): {path}")
            return 0
        print("final run on all curated cells (deployment artifact)")
        final = train(args.data, sets=args.sets, epochs=args.epochs,
                      size=args.size, batch=args.batch, lr=args.lr,
                      seed=args.seed, all_data=True, verbose=False)
        path = save_model(final, model_path(args.data),
                          meta={"metrics": metrics,
                                "trained": "all-data"})
        print(f"saved: {path}")
        return 0
