"""Synthetic split-flap fixtures shared by the ocr-vlm tests.
...
the glyph/classifier tests have a genuinely learnable (and safe to
assert on) task. ``make_baseline_set`` builds a verified baseline set
through the real curation API, and ``cache_payload`` fabricates a glyph
cache payload for unit tests that do not need photos at all.
"""
...
def synth_display(path, labels, *, w: int = 900, h: int = 220,
    ...
    simulates rig variance between shoots.
    """
...
def make_baseline_set(tmp_path, name: str, contents: dict,
                      *, jitter: int = 0):
    """Verified baseline set via the real curation API.

    ``contents`` maps photo filename -> 12-char content string. Photos
    are synthesized to show exactly that content.
    """
    ...
    (src / "report.json").write_text(json.dumps({"frames": frames}),
                                     encoding="utf-8")
    info = curate.create_set(str(src), name=name)
    for photo, content in contents.items():
        curate.update_entry(name, photo, content=content)  # verified
    return info
